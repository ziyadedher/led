...
pub async fn drive(config: RGBMatrixConfig, state: Arc<RwLock<State>>) -> anyhow::Result<()> {
    ...
    for _ in 0.. {
        ...
        canvas = block_in_place(|| -> anyhow::Result<Box<Canvas>> {
                        ...
                        for (j, c) in entry.text.chars().enumerate() {
                            ...
                            let char_offset = j as i32
                                * (font.character_size.width as i32
                                    + font.character_spacing as i32);
                            let rgb_progress = (((255 * 3) as f64
                                * (j as f64 / entry.text.len() as f64))
                                as usize
            ...
            Ok(matrix.update_on_vsync(canvas))
        })?;
    }
